...
The central rank, measured history, and interactive assumptions are distinct task surfaces. Controls keep ordinary web semantics. The original goblin illustration is representational art created with image generation, not an SVG icon substituted for art. The page uses black and grayscale throughout.

Desktop and mobile render inspection, chart focus, selection-bias controls, reset, data fetching, and overflow are checked before release. Design/release critique records and operational screenshots are private working artifacts, not published source data.
